      ...
      <div className="flex justify-between items-center font-['Fugaz One'] mx-4">
        ...
        <div className="flex items-center space-x-20 relative ">
          <div className="flex flex-col items-center pb-4">
            <div className="flex flex-col items-center relative md:ml-8 ml-4">
              <Link to="/">
                <img src="/logo1.png" alt="Logo" className="w-[55px] md:w-[60px] " />
              </Link>
              <p className="absolute md:text-[11px] text-center font-bold 
              ...
              <div className="absolute left-0 top-full mt-2 bg-white shadow-lg rounded-lg p-6 w-[600px] grid grid-cols-3 gap-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none group-hover:pointer-events-auto">
                {coursesData.map((category, index) => (
                  <div key={index} className="border-r border-gray-300 pr-4">
                    <h3 className="font-semibold text-orange-500 mb-4">{category.category}</h3>
                    <ul className="space-y-2 text-sm">
                      {category.courses.map((course, i) => (
                        <li key={i} className="hover:text-orange-500 transition cursor-pointer">{course}</li>
                      ))}
                    </ul>
                  </div>
                ))}
        ...
        <div className="flex items-center space-x-6">
          ...
          {loggedIn ? (
            ...
            <Link to="/login">
              <button className="px-5 py-2 border-2 border-orange-600 text-orange-600 font-semibold rounded-lg shadow-md hover:bg-orange-100 transition">Login</button>
            </Link>
            <Link to="/signup">
              <button className="px-5 py-2 bg-orange-600 text-white font-semibold rounded-lg shadow-md hover:bg-orange-700 transition">Sign Up</button>
            </Link>
          </div>
          )}
...
<motion.button
      ...
      {isMenuOpen && (
        <div className="md:hidden absolute top-20 left-0   w-[90%] bg-white/95 shadow-md rounded-lg p-4 mt-4 flex flex-col items-center space-y-4">
          ...
          <Link to="/courses" className="hover:text-orange-400 transition cursor-pointer">
            Courses
          </Link>
          {loggedIn ? (
            <>
              ...
              <Link to="/dashboard/helpandsupport" className="hover:text-orange-400 transition cursor-pointer">
                Help and Support
              </Link>
              <button onClick={logout} className="text-red-500 hover:text-red-700 transition cursor-pointer">
                Logout
              </button>
            </>
          ) : (
