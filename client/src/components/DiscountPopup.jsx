...
export default function DiscountPopup() {
  ...
  return (
    <Dialog open={showPopup} onOpenChange={handleClose}>
      {showPopup && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 backdrop-blur-md p-4 sm:p-6 z-50">
          <motion.div 
          ...
          >
            <DialogContent className="p-5 sm:p-6 rounded-xl shadow-xl bg-gradient-to-r from-orange-500 to-orange-400 text-white text-center border-4 border-orange-300">
              ...
              {offers.length > 0 && offers.map((offer) => {
                const hasBundle = offer.bundle && Object.keys(offer.bundle).length > 0;
                const discountAmount = hasBundle ? (offer.bundle.price - offer.bundle.discountedPrice).toFixed(1) : null;
                const discountPercentage = hasBundle ? offer.bundle.discount.toFixed(1) : null;
                const offerText = offer.text && offer.text.trim() !== ""
                  ? offer.text
                  : hasBundle
                    ? `${offer.bundle.bundleName} has ${discountPercentage}% discount!`
                    : "Exclusive offer!";
                ...
                return (
                  ...
                  >
                    <p className="text-lg sm:text-xl font-bold text-orange-600">
                      {hasBundle ? offer.bundle.bundleName : "Special Offer"}
                    </p>
